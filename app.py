...
with tabs[2]:
    ...
    with c2:
        fig2 = px.scatter(df, x='AI_Personalization_Score', y='Satisfaction_Rating',
                          ...
                          opacity=0.7)
        st.plotly_chart(styled_fig(fig2), use_container_width=True)
        r = float(np.corrcoef(df['AI_Personalization_Score'], df['Satisfaction_Rating'])[0, 1])
        st.markdown(f'<div class="insight-box"><strong>Pearson r = {r:.3f}</strong> — Strong positive correlation. Higher AI accuracy → higher user satisfaction across all subscription tiers.</div>', unsafe_allow_html=True)
    ...
    with c3:
        ...
        st.plotly_chart(styled_fig(fig3), use_container_width=True)
        r2 = float(np.corrcoef(df['AI_Personalization_Score'], df['CLV_INR'])[0, 1])
