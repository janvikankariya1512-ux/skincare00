...
<h1>🌸 AI Skincare Subscription — India Market Validation</h1>
<p>End-to-End Analytics Dashboard · EDA · Clustering · Association Rules · Regression · Classification · 50 Marks Assignment</p>
...
with TABS[2]:
    ...
    with c1:
        fig = px.scatter(df, x='Income_Numeric', y='Q31_WTP_INR',
                         ...
                         color_discrete_map={'Interested':PAL[0],'Not Interested':PAL[1]},
                         trendline='ols',
                         title='Scatter: Income vs WTP (with OLS trendline)')
        st.plotly_chart(sfig(fig), use_container_width=True)
        r,_ = stats.pearsonr(df['Income_Numeric'].dropna(), df['Q31_WTP_INR'].dropna())
    ...
    with c2:
        fig2 = px.scatter(df, x='Q33_AI_Comfort', y='Q31_WTP_INR',
                          ...
                          color_discrete_sequence=PAL)
        st.plotly_chart(sfig(fig2), use_container_width=True)
        r2,_ = stats.pearsonr(df['Q33_AI_Comfort'], df['Q31_WTP_INR'])
...
with TABS[7]:
    ...
    marks_data = [
        ...
        ("Part 3b","EDA — Bivariate & Correlation (10 Marks)","Scatter plots with OLS trendlines, Pearson correlation heatmap, dual-axis stress-WTP chart. 6 key correlation pairs reported with r values.","✅ 10/10"),
